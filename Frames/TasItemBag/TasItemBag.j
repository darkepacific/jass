...
// Banking: configurable Bank unit type rawcode (e.g., 'hban').
// Set this before game start (0 disables Bank detection).
private integer BankUnitType = 0

private function IsBankUnit takes unit u returns boolean
    return BankUnitType != 0 and GetUnitTypeId(u) == BankUnitType
endfunction
...
private function BagButtonAction takes nothing returns nothing
    ...
    if GetPlayerAlliance(GetOwningPlayer(Selected[pId]), p, ALLIANCE_SHARED_CONTROL) then
        ...
        local item it = BagItem[GetHandleId(Selected[pId])].item[bagIndex]
